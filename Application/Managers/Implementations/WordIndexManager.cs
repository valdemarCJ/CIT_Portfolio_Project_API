...
public class WordIndexManager : IWordIndexManager
{
    private readonly IWordIndexRepository _repo;
    private readonly IMapper _mapper;

    public WordIndexManager(IWordIndexRepository repo, IMapper mapper)
    {
        _repo = repo;
        _mapper = mapper;
    }
    ...
    {
        var pageDto = await _repo.GetWordIndexAsync(page, pageSize, ct);
        AddPageLinks(pageDto, "/api/word-index");
        foreach (var item in pageDto.Items)
        {
            item.Links.Add(new LinkDto("self", $"/api/word-index/{item.Tconst}/words/{item.Word}"));
        }
        return pageDto;
    }

    public async Task<List<WordIndexDto>> GetWordsByMovieAsync(string tconst, CancellationToken ct = default)
    {
        var entities = await _repo.GetWordsByMovieAsync(tconst, ct);
        var dtos = _mapper.Map<List<WordIndexDto>>(entities);
        foreach (var dto in dtos)
        {
            dto.Links.Add(new LinkDto("self", $"/api/word-index/{dto.Tconst}/words/{dto.Word}"));
        }
        return dtos;
    }
